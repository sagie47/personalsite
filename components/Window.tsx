...
const Window: React.FC<WindowProps> = ({
  ...
  const mobileStyles: React.CSSProperties = isMobile ? {
    position: 'fixed',
    inset: '0 0 calc(3rem + env(safe-area-inset-bottom)) 0',
    width: '100%',
    height: 'auto',
    maxHeight: 'calc(100dvh - 3rem - env(safe-area-inset-bottom))',
    zIndex: zIndex + 100,
  } : {
    left: pos.x,
    top: pos.y,
    ...
    >
      ...
      <div
        className={`px-2 py-1.5 flex items-center justify-between select-none mr-0.5 mt-0.5 ml-0.5
          ${isMobile ? 'cursor-default py-3 pr-1' : 'cursor-grab active:cursor-grabbing'}
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-bold text-sm sm:text-base tracking-wide ml-1 text-white font-ms-sans truncate max-w-[58vw] sm:max-w-none">
            {title}
          </span>
        </div>
        ...
        <button
          ...
          className="window-action-button w-10 h-10 sm:w-5 sm:h-5 bg-[#c0c0c0] border border-t-white border-l-white border-r-black border-b-black flex items-center justify-center active:border-t-black active:border-l-black active:border-r-white active:border-b-white focus:outline-none touch-manipulation"
      ...
      {/* Content Area */}
      <div className="flex-1 flex flex-col overflow-hidden relative p-1 min-h-0 overscroll-contain">
        {children}
      </div>
    </div>
  );
